...
import { generateRecipe } from "@/lib/ai-recipe";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(`食谱生成成功: ${recipe.name}, 总耗时: ${Date.now() - startTime}ms`);

    return NextResponse.json(recipe);
  } catch (error) {
    const totalTime = Date.now() - startTime;
    console.error(`食谱生成失败，总耗时: ${totalTime}ms`, error);

    let status = 500;
    let message = "食谱生成失败，请稍后再试";
